...
{
   public class SNQReport:ISNQReport
    {
        #region
        ...
        public SNQReport(DataConnection datacontext, IConfiguration configuration)
        {
        ...
        public List<SNQReportSummary> GetSNQDaily_Report(DailyReport objDailyReport)
        {
            #region
            ...
            {
                SNQReportSummary objSNQReportSummary = new SNQReportSummary
                {
                    ...
                    detailCount = (from dtl in _datacontext.TSNQ_ENQUIRY_DTLTable where dtl.FK_SNQENQUIRY_HDR_ID == item.PK_SNQENQUIRY_HDR_ID select dtl).ToList().Count(),
                    errorDetailCount = (from dtl in _datacontext.TSNQ_ENQUIRY_DTLTable where (dtl.FK_SNQENQUIRY_HDR_ID == item.PK_SNQENQUIRY_HDR_ID && dtl.STATUS == 5) select dtl).ToList().Count(),
                ...
                lstSNQReportSummary.Add(objSNQReportSummary);
            }
            #endregion

            #region
            if (lstSNQReportSummary.Count != 0 && isExcelCreate)
            {

                CreateExel(lstSNQReportSummary, Todate, toDate, fromDate);
            ...
            #endregion
            return lstSNQReportSummary.OrderByDescending(x=>x.emailReceivedat).ToList();
        }

        public void CreateExel(List<SNQReportSummary> lstSNQReportSummary, string today, DateTime FROMDATE, DateTime TODATE)
        {
            try
            {
                #region
                string outputPath = this._Configuration.GetSection("ReportSettings")["SNQOutputPath"];
                #endregion
        ...
        public void Send_Mail(string filepath)
        {
            try
            {
                ...
                #region
                string reportEnv = this._Configuration.GetSection("SNQMailSettings")["Enviornment"];

                MailMessage mail = new MailMessage();
              
                SmtpClient SmtpServer = new SmtpClient("smtp.gmail.com");

                mail.From = new MailAddress(this._Configuration.GetSection("SNQMailSettings")["FromMail"]);
                mail.To.Add(this._Configuration.GetSection("SNQMailSettings")["ToMail"]);
                mail.CC.Add(this._Configuration.GetSection("SNQMailSettings")["CCMail"]);


                mail.Subject = "Daily " + reportEnv + " SNQ quotation creation report";
                ...
                SmtpServer.Credentials = new System.Net.NetworkCredential(this._Configuration.GetSection("SNQMailSettings")["Username"], this._Configuration.GetSection("SNQMailSettings")["Password"]);
            ...
            catch (Exception ex)
            {
                ...
                string result = "";
                result = "send mail  error" + ex.Message ;
            }
        }
